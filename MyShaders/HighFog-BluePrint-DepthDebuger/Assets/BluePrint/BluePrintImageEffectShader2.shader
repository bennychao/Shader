Shader "Hidden/BluePrintImageEffectShader2"
{
	Properties
	{
...
//			}	
			...
			{
				half4 sample1 = tex2D(_MainTex, uv1);
				//then d is 0~1
				...
				half4 sample2 = tex2D(_MainTex, uv2);
				//then d is 0~1
...
//				}
		...
		//pass 1 
		//merge blueprint layer and the background
		Pass
		{
			CGPROGRAM
			#pragma vertex vert
			...
			inline float NormalDotDistance(sampler2D tex, float2 uv1, float2 uv2)	//, out float dotDistance
			{
				half4 sample1 = tex2D(tex, uv1);
				//then d is 0~1
...
//				}
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				float dotDistance = NormalDotDistance(_MainTex2, i.uv[1], i.uv[2]);
				float dotDistance2 = NormalDotDistance(_MainTex2, i.uv[3], i.uv[4]);

				//dotDistance = (dotDistance + dotDistance2) / 2;

				float dotDistance3 = NormalDotDistance(_MainTex3, i.uv[1], i.uv[2]);
				float dotDistance4 = NormalDotDistance(_MainTex3, i.uv[3], i.uv[4]);

				//dotDistance = (dotDistance3 + dotDistance4) / 2;
				dotDistance = (dotDistance + dotDistance2 + dotDistance3 + dotDistance4) / 4;
